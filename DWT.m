...
[h,g]= wfilters('sym8','d');       %  分解低通和高通滤波器   ‘d’返回分解滤波器 sym8小波
...
for jj=rank_min:rank_max
    ...
    nn=2^jj;
    %  构造向量
    p1_0=sparse([h,zeros(1,nn-L)]);   %  zeros生成1行nn-L列的矩阵  sparse创造稀疏矩阵
    p2_0=sparse([g,zeros(1,nn-L)]);   %  
    
    %  向量圆周移位
    for ii=1:nn/2
        p1(ii,:)=circshift(p1_0',2*(ii-1))';   %   Y = circshift(A,K) 循环将 A 中的元素列平移 K 个位置。
    ...
    w=sparse([w1,zeros(length(w1),mm);zeros(mm,length(w1)),eye(mm,mm)]);    %  eye创建mm*mm的单位矩阵
    ww=ww*w;
